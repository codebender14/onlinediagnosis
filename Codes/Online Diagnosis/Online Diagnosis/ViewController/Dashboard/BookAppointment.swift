...
class BookAppointment: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if viewAppointment && !viewApproveAppointment{
            if appointmentList?.status != "Pending" || doctorViewAppointment {
                self.disableUserInteraction()
                self.pendingAppointmentData()
            } else {
                self.pendingAppointmentData()
            }
    ...
    @IBAction func onDelete(_ sender: Any){
         ...
         //   let alertController = UIAlertController(title: "", message: "Are you sure you want to delete this appointment?", preferredStyle: .alert)
            
            // Create the first button (e.g., OK)
